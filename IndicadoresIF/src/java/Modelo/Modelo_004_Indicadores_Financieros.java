...
public class Modelo_004_Indicadores_Financieros {
    ...
    public static String Consulta_Tabla_Indicadores_Financieros(Integer anio, String mes) {
        return "select Nindicador,\n"
                ...
                + "\n"
                + "sum(CASE WHEN Empresa=4 then valor else 0 end) as 'Imperialtex',\n"
                + "\n"
                + "ifnull((select sum(valor)/count(valor) from I_010_Financieros where empresa = 4\n"
                + "AND MES = P.MES AND ANIO=" + (anio - 1) + "),0) AS 'pit',\n"
                + "\n"
                + "sum(CASE WHEN Empresa=5 then valor else 0 end) as 'Fabricaimp',\n"
                + "\n"
                + "ifnull((select sum(valor)/count(valor) from I_010_Financieros where empresa = 5\n"
                + "AND MES = P.MES AND ANIO=" + (anio - 1) + "),0) AS 'pfi',\n"
                + "\n"
                + "sum(CASE WHEN Empresa=6 then valor else 0 end) as 'Consolidado',\n"
                + "\n"
                + "ifnull((select sum(valor)/count(valor) from I_010_Financieros where empresa = 6\n"
                + "AND MES = P.MES AND ANIO=" + (anio - 1) + "),0) AS 'pcn'\n"
                + "\n"
