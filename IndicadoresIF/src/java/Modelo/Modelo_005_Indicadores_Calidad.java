...
public class Modelo_005_Indicadores_Calidad {
    ...
    public static String Consulta_Tabla_Indicadores_Calidad(int anio, String mes) {
        return "Select \n"
                + "ambito, \n"
                + "indicador,\n"
                + "sum(case when planta ='RS'   then valor else 0 end) as 'RS',\n"
    ...
    public static String Consulta_Calidad_General(int anio, String mes, String indicador) {
        ...
        return "SELECT  planta,\n"
                ...
                + "\n"
                + "FROM   I_011_Indicadores_Calidad P\n"
                + "where P.MES=" + mes + " and P.indicador='" + indicador + "' GROUP BY P.planta ";
    }
    ...
    public static String Consulta_Calidad_Detalle(Integer anio, String indicador, String planta, String amb) {
        ...
        return "SELECT  \n"
                ...
                + "	AND planta = P.planta AND P.indicador=indicador ORDER BY VALOR DESC LIMIT 1) AS 'mejor',\n"
                + "                \n"
                + "(select VALOR from I_011_Indicadores_Calidad\n"
                + "	where VALOR in (select VALOR from I_011_Indicadores_Calidad where  planta = P.planta)\n"
                + "	AND planta = P.planta AND P.indicador=indicador ORDER BY VALOR ASC LIMIT 1) AS 'peor',\n"
                + "                \n"
                + "                \n"
                + "ifnull(\n"
                + "	(select sum(valor)/count(valor) from I_011_Indicadores_Calidad \n"
                + "    where planta = P.planta AND P.indicador=indicador AND  ANIO=" + (anio - 1) + " ) , 0) AS 'promedio',\n"
                + "\n"
                + "(select mes from I_011_Indicadores_Calidad\n"
                + "	where VALOR in (select VALOR from I_011_Indicadores_Calidad where  planta = P.planta)\n"
                + "	AND planta = P.planta AND P.indicador=indicador  ORDER BY VALOR DESC LIMIT 1) AS 'mejormes',\n"
                + "\n"
                + "(select anio from I_011_Indicadores_Calidad\n"
                + "	where VALOR in (select VALOR from I_011_Indicadores_Calidad where  planta = P.planta)\n"
                + "	AND planta = P.planta AND P.indicador=indicador  ORDER BY VALOR DESC LIMIT 1) AS 'mejoranio',\n"
                + "                \n"
                + "(select mes from I_011_Indicadores_Calidad\n"
                + "	where VALOR in (select VALOR from I_011_Indicadores_Calidad where  planta = P.planta)\n"
                + "	AND planta = P.planta AND P.indicador=indicador  ORDER BY VALOR ASC LIMIT 1) AS 'peormes',\n"
                + "\n"
                + "(select anio from I_011_Indicadores_Calidad\n"
                + "	where VALOR in (select VALOR from I_011_Indicadores_Calidad where  planta = P.planta)\n"
                + "	AND planta = P.planta AND P.indicador=indicador  ORDER BY VALOR ASC LIMIT 1) AS 'peoranio'\n"
                + "\n"
                + "FROM   I_011_Indicadores_Calidad P\n"
